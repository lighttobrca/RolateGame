...
bool playground::init()
{
    ...
    this->runAction(Sequence::create(Spawn::create(CallFunc::create([this]{
        this->scheduleUpdate();
    }) ,/*DelayTime::create(0.5133)*/DelayTime::create(0.2),NULL),CallFunc::create([this]{
...
void playground::circlePlayer(float dt)
{
    ...
    if (isHit){
        if (_img->getTexture() == _gold->getTexture()) {
            this->removeChildByTag(0);
            addGold(size, dt);
            _score += 1;
        }
        else if(_img->getTexture() == _rotateTurbo->getTexture()){
            this->removeChildByTag(0);
            addGold(size, dt);
            _score += 5;
        }
